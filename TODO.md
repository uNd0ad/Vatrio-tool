...
- [ ] **Pagination & Infinite Scroll:** Implement server-side pagination for `fetchListings()` in the desktop app. Currently, it fetches all rows at once, which will slow down as the database grows.
- [ ] **On-Demand Data Loading:** Load listing details (like full notes and histories) only when a row is selected.
...
- [ ] **Anti-Detection & Proxy Rotation:** Add proxy support and stealth browser plugins (e.g., `playwright-extra` with stealth plugin) to prevent target websites (OLX, Storia, Imobiliare) from blocking scraper runs.
...
- [ ] **Storia.ro Scraping Integration:** Implement the Playwright crawler for `storia.ro` (under `crawler/src/sites/storia.ts`) to complete the coverage of all supported property portals.
- [ ] **HomeZZ & Publi24 Scraping Integration:** Expand the crawler to support `homezz.ro` and `publi24.ro` real estate platforms (which will also require updating the database schema check constraint for allowed sources).
...
- [ ] **Deduplication Engine:** Detect identical property ads posted on different platforms (or multiple times by different agencies) using price, surface area, and location proximity.

### 3. CRM Desktop App Feature Enhancements (`/src`)
- [ ] **Real-time Updates:** Use Supabase Realtime subscriptions to automatically inject new scraper listings into the UI without needing to press the "Actualizează" (Refresh) button.
- [x] **Dark Mode Support:** Implement a theme switch system in `app.css`.
- [ ] **Bulk Actions:** Allow checking multiple rows to change status (e.g., set status to "Refuzat" for 10 listings at once).
- [ ] **Visual Analytics Page:** Add a dashboard tab showing:
...
- [ ] **Activity Logs:** Create an audit trail to log which member contacted/updated a listing (who, what, when).
