  ...
  return (
    <>
      ...
      <aside className="detail-drawer">
        ...
        <div className="drawer-badges"><SourceMark source={listing.source}/><span className={`seller-badge ${listing.seller_type}`}>{sellerTypeLabel(listing.seller_type)}</span><span className="seller-badge" style={{ background: listing.transaction_type === "sale" ? "#e8f0fe" : "#f3e8ff", color: listing.transaction_type === "sale" ? "#1a73e8" : "#7c3aed", fontWeight: 800 }}>{listing.transaction_type === "sale" ? "De Vânzare" : "De Închiriat"}</span></div><h2>{listing.title}</h2><p className="drawer-price">{formatPrice(listing)}{formatPricePerSqm(listing.price, listing.surface_sqm, listing.currency) && <span style={{ fontSize: "13px", fontWeight: 400, color: "var(--text-muted)", marginLeft: "10px" }}>({formatPricePerSqm(listing.price, listing.surface_sqm, listing.currency)})</span>}</p><p className="drawer-location"><Icon name="pin"/>{listing.location ?? "Nespecificată"}</p>
